    ...
    app.post('/deliveryWork', function (req, res) {
        console.log(`/POST-deliveryWork - Token: " + ${req.body.token}, Zip File:  ${req.files.fileInput.name}`)
            ...
            if (deliveriedRes.tokenExist) {
                console.log(`/POST-deliveryWork - studantName:${deliveriedRes.studantName},projectName: ${deliveriedRes.projectName}`)
                res.render('deliveryWork', { successMsg: `Olá ${deliveriedRes.studantName}, seu trabalho '${deliveriedRes.projectName}' foi entregue com sucesso :)`, errorMsg: null, token: '' });
            } else {
